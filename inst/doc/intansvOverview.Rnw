...
Currently, dozens of softwares have been developped to predict structural
...
The example dataset was obtained by running five different softwares with the 
...
These five softwares including BreakDancer \citep{BreakDancer}, 
...
\subsection{Read in predictions of different softwares}

Most of the predictions of softwares are tedious and need to be formatted 
for further analysis. \intansv{} provides functions to read in the 
predictions of five popular softwares, BreakDancer, Pindel, CNVnator, 
...
Note that files in the directories that contain the predictions of CNVnator, 
DELLY, Pindel and SVseq2 should be named with approriate suffix names. See 
the manuals and the example dataset for more details.

The results of these five softwares have now been read into R and storaged as 
...
CNVnator is able to predict deletions and duplications.
...
\subsection{Integrate predictions of different softwares}

To get more reliable results, we need to integrate the predictions of
different softwares. See our paper \citep{intansv} for more details on 
how \intansv{} integrate the predictions of different softwares.
<<MethodsMerge>>=
...
\intansv{} is also able to integrate SV predictions of other softwares. However,
predictions of other softwares should be provided as a data frame with six columns:
...
\item{methods}{\qquad the software used to get this SV prediction}
\end{itemize}
